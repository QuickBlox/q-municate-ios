...
- (id <SDWebImageOperation>)downloadImageWithURL:(NSURL *)url
    ...
    qm_cache_operation cacheOp = ^() {
        return [SDImageCache.sharedImageCache queryCacheOperationForKey:key done:^(UIImage * _Nullable image,
            ...
            if ((!image || options & SDWebImageRefreshCached) &&
                (![weakSelf.delegate respondsToSelector:@selector(imageManager:shouldDownloadImageForURL:)] ||
                 [weakSelf.delegate imageManager:weakSelf shouldDownloadImageForURL:url])) {
                    ...
                    [SDWebImageDownloader.sharedDownloader downloadImageWithURL:urlToDownload
                ...
                if (transform) {
                    ...
                    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
                        SDImageCache *qmCache = (SDImageCache *)weakSelf.imageCache;
                        UIImage *transformedImage = [qmCache imageFromDiskCacheForKey:transformKey];
    ...
    if (transform) {
        SDImageCache *qmCache = (SDImageCache *)weakSelf.imageCache;
        [qmCache queryCacheOperationForKey:transformKey
...
- (UIImage *)originalImageWithURL:(NSURL *)url {
    SDImageCache *qmCache = (SDImageCache *)self.imageCache;
    return [qmCache imageFromDiskCacheForKey:url.absoluteString];
}
    ...
    if (transform) {
        ...
        UIImage *transformedImage = [transform imageManager:imageManager
                                   transformDownloadedImage:image
                                                    withURL:imageURL];
        SDImageCache *qmCache = (SDImageCache *)self.imageCache;
        [qmCache storeImage:transformedImage
                  imageData:nil
                     forKey:transformKey
                     toDisk:YES
                 completion:nil];
...
- (BOOL)hasOriginalImageWithURL:(NSURL *)url {
    ...
    NSString *key = [self cacheKeyForURL:url];
    SDImageCache *qmCache = (SDImageCache *)self.imageCache;
    NSString *path = [qmCache cachePathForKey:key];
    if (path) {
