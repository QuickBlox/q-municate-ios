...
- (void)initialGradientOrientation {
    ...
    switch (self.progressType) {
        ...
        case QMProgressTypeVertical: [self setGradientOrientationAngle:M_PI_2]; break;
        case QMProgressTypeHorizontal: [self setGradientOrientationAngle:0]; break;
            
        default:
            NSAssert(self.progressType == QMProgressTypeNone, @"!You need to set the meterType property to something else than QMProgressTypeNone");
            break;
    };
}
...
- (CALayer *)mask {
    ...
    return self.gradientLayer.mask;
}

- (void)setProgressType:(QMProgressType)progressType {
...
- (void)updateGradientColors {
    ...
    self.gradientLayer.colors = @[(id)self.progressTintColor.CGColor, (id)self.trackTintColor.CGColor];
...
- (CGFloat)rescaledProgress:(CGFloat)progress {
    ...
    CGRect shapeBounds = self.mask.bounds;
...
+ (NSArray *)intersectionPointsOfLineOrientedBy:(CGFloat)angle withBox:(CGRect)box {
    ...
    if (fabs(angle - M_PI_2) < FLT_EPSILON) {
        // vertical "up"
    ...
    } else if (fabs(angle - 3*M_PI_2) < FLT_EPSILON) {
        // vertical "down"
    ...
    } else if (fabs(fabs(angle) - 2*M_PI_2) < FLT_EPSILON) {
        // horizontal "left"
...
- (NSArray *)gradientPoints:(CGFloat)angle {
    ...
    NSArray *intersectionPoints = [QMProgressView intersectionPointsOfLineOrientedBy:angle withBox:box];
