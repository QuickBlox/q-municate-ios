...
+ (NSString *)messageForText:(NSString *)text participants:(NSArray *)participants
{
    ...
    return [NSString stringWithFormat:text, [QMCore instance].currentUser.fullName, addedUsersNames];
}
...
+ (NSArray *)occupantsWithoutCurrentUser:(NSArray *)occupantIDs {
    ...
    for (NSNumber *identifier in occupantIDs) {
        if (![identifier isEqualToNumber:@([QMCore instance].currentUser.ID)]) {
